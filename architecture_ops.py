...
def _conv(inputs, filters, kernel_size=1, strides=1, pad='VALID', name='conv'):
    with tf.variable_scope(name):
        ...
        kernel = tf.get_variable(shape=[kernel_size, kernel_size, inputs.get_shape().as_list()[3], filters],
                                 initializer=tf.contrib.layers.xavier_initializer(uniform=False), name='weights')
        conv = tf.nn.conv2d(inputs, kernel, [1, strides, strides, 1], padding=pad, data_format='NHWC')
        return conv


def _conv_bn_relu(inputs, filters, train, kernel_size=1, strides=1, pad='VALID', name='conv_bn_relu'):
    with tf.variable_scope(name):
        kernel = tf.get_variable(shape=[kernel_size, kernel_size, inputs.get_shape().as_list()[3], filters],
                                 initializer=tf.contrib.layers.xavier_initializer(uniform=False), name='weights')
        conv = tf.nn.conv2d(inputs, kernel, [1, strides, strides, 1], padding=pad, data_format='NHWC')
        norm = tf.nn.relu(tf.layers.batch_normalization(conv, momentum=0.9, epsilon=1e-5, training=train, name='bn'))
        return norm
...
def _conv_block(inputs, numOut, train, name='conv_block'):
    with tf.variable_scope(name):
        with tf.variable_scope('norm_1'):
            norm_1 = tf.nn.relu(tf.layers.batch_normalization(inputs, momentum=0.9, epsilon=1e-5, training=train, name='bn'))

            conv_1 = _conv(norm_1, int(numOut / 2), kernel_size=1, strides=1, pad='VALID', name='conv')
        with tf.variable_scope('norm_2'):
            norm_2 = tf.nn.relu(tf.layers.batch_normalization(conv_1, momentum=0.9, epsilon=1e-5, training=train, name='bn'))

            pad = tf.pad(norm_2, np.array([[0, 0], [1, 1], [1, 1], [0, 0]]), name='pad')
            conv_2 = _conv(pad, int(numOut / 2), kernel_size=3, strides=1, pad='VALID', name='conv')
        with tf.variable_scope('norm_3'):
            norm_3 = tf.nn.relu(tf.layers.batch_normalization(conv_2, momentum=0.9, epsilon=1e-5, training=train, name='bn'))
