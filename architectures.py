...
def discriminator_patch(image, train):
    padding = 'VALID'
    x0 = image
    x1 = tf.layers.conv2d(x0, 32, 4, strides=1, padding=padding, activation=tf.nn.leaky_relu, name="conv_0")  # 46
    x1 = tf.layers.batch_normalization(x1, training=train, name='bn_0')
    x1 = tf.layers.conv2d(x1, 64, 4, strides=2, padding=padding, activation=tf.nn.leaky_relu, name="conv_1")  # 44
    x1 = tf.layers.batch_normalization(x1, training=train, name='bn_1')
    x2 = tf.layers.conv2d(x1, 128, 4, strides=2, padding=padding, activation=tf.nn.leaky_relu, name="conv_2")  # 10
    x2 = tf.layers.batch_normalization(x2, training=train, name='bn_2')
    x3 = tf.layers.conv2d(x2, 256, 4, strides=2, padding=padding, activation=tf.nn.leaky_relu, name="conv_3")  # 4
...
def decoder(encoding_list, train, reconstr_dim, n_c):
    ...
    if reconstr_dim == 128:
        conv4 = nccuc(conv3, encoding_list[-5], [256, 128], padding, train, name=4)  # 64
        conv5 = nccuc(conv4, encoding_list[-6], [128, 64], padding, train, name=5)  # 128
        conv6 = tf.layers.conv2d(conv5, n_c, 6, strides=1, padding='SAME', activation=tf.nn.sigmoid, name="conv_6")
        reconstruction = conv6  # 128

    if reconstr_dim == 256:
        conv4 = nccuc(conv3, encoding_list[-5], [256, 128], padding, train, name=4)  # 64
        conv5 = nccuc(conv4, encoding_list[-6], [128, 128], padding, train, name=5)  # 128
        conv6 = nccuc(conv5, encoding_list[-7], [128, 64], padding, train, name=6)  # 256
        conv7 = tf.layers.conv2d(conv6, n_c, 6, strides=1, padding='SAME', activation=tf.nn.sigmoid, name="conv_7")
